...
import resumeEnData from "@/app/_components/resume_en.json";
import ResumeZhData from "@/app/_components/resume_zh.json";
...
export default function Resume({initialLocale}: { initialLocale: 'en' | 'zh' }) {
  const [locale, setLocale] = useCookieState('locale', { defaultValue: initialLocale });

  const data = locale === 'zh' ? ResumeZhData : resumeEnData

  // Set CSS custom property for page header
  if (typeof document !== "undefined" && data.pageHeader) {
